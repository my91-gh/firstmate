...
Both the pull-based and push-based guards are in-band, so neither runs when the host low-memory guard reaps the supervision process during an idle window with no turn to fire.
An out-of-band liveness guard (`bin/fm-supervision-guard.sh`) closes that gap: it runs in a detached terminal host, re-checks the model-aware verdict for its own home, and either relaunches a reaped away daemon or raises a loud durable alarm, so a reaped supervisor no longer stays silent (documented in [supervision-guard.md](supervision-guard.md)).
