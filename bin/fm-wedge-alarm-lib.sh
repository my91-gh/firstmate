...
# Backend-independent active-alert library, shared by the away-mode sub-supervisor
# (bin/fm-supervise-daemon.sh) and the supervision liveness guard
# (bin/fm-supervision-guard.sh).
#
# It resolves the configured alert channels and posts a loud, backend-independent
# active alert that does not depend on any pane or its multiplexer status-line: an
# OS-level macOS notification, a herdr notification, or a captain-supplied command
# (push to a phone/pager). Each caller keeps its own durable marker, rate-limiting,
# and any cosmetic status-line flash; this library owns only the active-alert
# channel resolution and dispatch, so both callers share one notification contract.
#
...
# single directive. Directives (docs/wedge-alarm.md owns the operator contract):
...
# An absent config means auto, i.e. default-ON on macOS: the alarm's whole purpose
# is to never be silent, so the reachable OS channel fires unless disabled.
#
# Dependencies the caller provides: a `log` function (this library installs a
# stderr fallback only when none is already defined, so the daemon's file logger
# and the guard's own logger both keep working). WEDGE_ALARM_TITLE sets the
# notification title; wedge_alarm_notify accepts it as an optional argument and
# restores the previous value, so the daemon keeps its away-mode title and the
# guard supplies its own without either leaking into the other.
#
# Test seam: FM_WEDGE_ALARM_EXEC, when set, REPLACES the real notifier so no test
# can post a real desktop notification. "discard" fires nothing; any other value
# runs that command with the channel name and summary; unset means production.

: "${WEDGE_ALARM_TIMEOUT_SECS_DEFAULT:=10}"
WEDGE_ALARM_NOTIFIER_PID="${WEDGE_ALARM_NOTIFIER_PID:-}"
: "${WEDGE_ALARM_TITLE:=firstmate: away-mode escalations WEDGED}"

# Install a stderr fallback logger only when the caller has not defined one, so a
# caller that sources this before defining `log` (the daemon defines it later) is
# still safe, and its own `log` definition overrides this afterward.
if ! command -v log >/dev/null 2>&1; then
  log() { printf 'fm-wedge-alarm: %s\n' "$*" >&2; }
fi
...
# or herdr or a captain-supplied command. This is the one injection point the test
# harness forces to a recorder so no test can post a real desktop notification.
# The special value "discard" fires nothing; unset means production, so the real
# channels fire.
...
wedge_alarm_via_osascript() {  # <summary>
  ...
  wedge_alarm_run_bounded osascript osascript -e 'on run argv' \
    -e "display notification (item 1 of argv) with title \"$WEDGE_ALARM_TITLE\" sound name \"Basso\"" \
...
wedge_alarm_via_herdr() {  # <summary>
  ...
  wedge_alarm_run_bounded herdr herdr notification show "$WEDGE_ALARM_TITLE" \
    --body "$summary" --sound request >/dev/null 2>&1 && return 0
  log "wedge alarm: herdr notification failed"
  return 1
}

# Run a captain-supplied command with the summary on $1 and on stdin, so an alert
# can reach a phone/pager (ntfy, Slack, SMS) even when the captain is away from
# the machine entirely. Best-effort: logs and returns 1 on failure.
...
# channel failure can never abort the caller's alarm path or a daemon loop. Any
# `off` directive disables the alert, regardless of position; an unresolvable
# `auto` (no OS channel on this platform) logs that the durable marker is the only
# signal. Every notifier routes through the test-forced recorder seam. An optional
# third argument overrides the notification title for this call only.
wedge_alarm_notify() {  # <summary> <marker> [title]
  local summary=$1 marker=$2 title=${3:-} ch prev_title=$WEDGE_ALARM_TITLE
  local -a channels=()
  [ -n "$title" ] && WEDGE_ALARM_TITLE=$title
