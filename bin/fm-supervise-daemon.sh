...
# Backend-independent active-alert channel resolution and dispatch, shared with
# the supervision liveness guard. This daemon keeps its own durable marker,
# rate-limiting, and tmux status-line flash and calls wedge_alarm_notify for the
# active alert. Sourced before `log` is defined; the library installs a stderr
# fallback that this daemon's own `log` (defined later) overrides.
...
# bin/fm-wedge-alarm-lib.sh (sourced above). WEDGE_ALARM_LAST_EPOCH is this
# daemon's own once-per-window rate-limit clock for inject_wedge_alarm.
